'''
Implementataion of STAN model
Most of the code is from https://github.com/yingtaoluo/Spatial-Temporal-Attention-Network-for-POI-Recommendation
...
def to_npy(x):
    # return x.cpu().data.numpy() if device == 'cuda:2' else x.detach().numpy()
...
class Attn(nn.Module):
    ...
    def forward(self, self_attn, self_delta, traj_len, device):
...
class MultiEmbed(nn.Module):
    ...
    def forward(self, traj, mat, traj_len):
        ...
        traj[:, :, 2] = (traj[:, :, 2]) % hours  # segment time by 24 hours * 7 days
...
class STAN(nn.Module):
    def __init__(self, t_dim, l_dim, u_dim, embed_dim, ex, device, dropout=0.1):
        super(STAN, self).__init__()
        emb_t = nn.Embedding(t_dim, embed_dim)
        emb_l = nn.Embedding(l_dim, embed_dim)
        emb_u = nn.Embedding(u_dim, embed_dim)
    ...
    def forward(self, traj, mat1, mat2, vec, traj_len):
        ...
        output = self.Attn(self_attn, self_delta, traj_len, self.device)  # (N, L)
        return output